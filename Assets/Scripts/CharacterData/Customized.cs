...
public class Customized : MonoBehaviour
{
    ...
    void Init()
    {
        ...
        {
            GamaMan.Inst.userPartyCheck[System.Convert.ToInt32(gameObject.name.Replace("Character", ""))] = true;
    ...
    public void AsyncParts(int index)
    {
        ...
        {
            if (GamaMan.Inst.temp[i, index] != null)
            {
                parts[i].sprite = GamaMan.Inst.temp[i, index];
            }
        }
        parts[0].color = GamaMan.Inst.partsColor[0, index];
        parts[1].color = GamaMan.Inst.partsColor[1, index];
        parts[2].color = GamaMan.Inst.partsColor[2, index];
    }
}
